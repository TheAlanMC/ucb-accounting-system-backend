...
class SupplierApi @Autowired constructor(private val supplierBl: SupplierBl){
    ...
    fun getSuppliers(@PathVariable companyId: Long): ResponseEntity<ResponseDto<List<SupplierPartialDto>>> {
        logger.info("Starting the API call to get suppliers")
        logger.info("GET /api/v1/suppliers/companies/$companyId")
        val suppliers: List<SupplierPartialDto> = supplierBl.getSuppliers(companyId)
        logger.info("Sending response")
        val code = "200-30"
        val responseInfo = ResponseCodeUtil.getResponseInfo(code)
        logger.info("Code: $code - ${responseInfo.message}")
        return ResponseEntity(ResponseDto(code, responseInfo.message!!, suppliers), responseInfo.httpStatus)
    }
